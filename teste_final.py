...
st.markdown("""
    ...
    .report-box {
        ...
        border: 5px solid #FFCC00; border-radius: 10px;
    }
...
with col_titulo:
    st.markdown("""
        <div style="padding-top: 25px;">
            <h1 style="color:#FFCC00; font-family:'Arial Black'; font-size:2.2rem; margin-bottom:0;">
                ⚖️ CALCULADORA DO TRECHO
            </h1>
            <p style="color:#FFCC00; font-size:1.1rem; margin-top:5px;">
...
municipios_rmsp = [" "] + sorted([
    ...
    "São Bernardo do Campo", "São Caetano do Sul", "São Lourenço da Serra", "São Paulo",
    "Suzano", "Taboão da Serra", "Vargem Grande Paulista"
])
distritos_sp = [" "] + sorted([
    "Água Rasa", "Alto de Pinheiros", "Anhanguera", "Aricanduva", "Artur Alvim",
    "Barra Funda", "Bela Vista", "Belém", "Bom Retiro", "Brasilândia", "Butantã",
    "Cachoeirinha", "Cambuci", "Campo Belo", "Campo Grande", "Campo Limpo", "Cangaíba",
    "Capão Redondo", "Carrão", "Casa Verde", "Cidade Ademar", "Cidade Dutra",
    ...
    "Jardim Ângela", "Jardim Helena", "Jardim Paulista", "Jardim São Luís", "Lapa",
    "Liberdade", "Limão", "Mandaqui", "Marsilac", "Moema", "Mooca", "Morumbi",
    "Parelheiros", "Pari", "Parque do Carmo", "Pedreira", "Penha", "Perdizes", "Perus",
    "Pinheiros", "Pirituba", "Ponte Rasa", "Raposo Tavares", "República", "Rio Pequeno",
    "Sacomã", "Santa Cecília", "Santana", "Santo Amaro", "São Domingos", "São Lucas",
    "São Mateus", "São Miguel", "São Rafael", "Sapopemba", "Saúde", "Sé", "Socorro",
    "Tatuapé", "Tremembé", "Tucuruvi", "Vila Andrade", "Vila Curuçá", "Vila Formosa",
    "Vila Guilherme", "Vila Jacuí", "Vila Leopoldina", "Vila Maria", "Vila Mariana",
...
escolaridade = p4.selectbox("ESCOLARIDADE", [
    ...
    "Médio Completo", "Técnico", "Superior Incompleto", "Superior Completo", "Pós-Graduação"
])
setor = p5.selectbox("SETOR DE ATIVIDADE", [
    "Comércio", "Construção Civil", "Educação", "Indústria", "Serviços", "Saúde", "Outros"
])
...
# ─────────────────────────────────────────────
# 6. BOTÃO — salva no session_state
# ─────────────────────────────────────────────
if st.button("📊 EFETUAR DIAGNÓSTICO"):
    if mun_moradia == " " or sal <= 0:
        ...
        st.session_state["resultado"] = {
            ...
            "dist_moradia": dist_moradia if dist_moradia and dist_moradia != " " else mun_moradia,
            "mun_trabalho": mun_trabalho,
            "dist_trabalho": dist_trabalho if dist_trabalho and dist_trabalho != " " else mun_trabalho,
        }
        st.session_state["salvo"] = False

# ─────────────────────────────────────────────
# 7. RESULTADOS — fora do if st.button
# ─────────────────────────────────────────────
if "resultado" in st.session_state:
    ...
    st.markdown(f"""
        <div style="background:#000; padding:20px; border:2px solid #E63946; text-align:center; margin-bottom:20px;">
    ...
    st.markdown(f"""
        <div class="report-box">
            ...
            <p>• 📉 <b>DEPRECIAÇÃO REAL:</b>
                <span style="color:{cor_alerta}; font-weight:900; font-size:1.4rem;">{depre:.1f}%</span>
            </p>
        </div>
    """, unsafe_allow_html=True)

    st.markdown(f"""
        <div style="background-color:#FFFFFF; padding:25px; border-left:12px solid #FFCC00;
                    border:1px solid #E0E0E0; border-radius:8px; margin-top:25px;">
            ...
            <div style="color:#333333; font-size:1.1rem; line-height:1.6; text-align:justify;">
                ...
                O trecho é <b>trabalho não pago</b>: tempo obrigatório para a reprodução da força
                de trabalho que gera uma depreciação de
                <span style="color:#E63946; font-weight:bold;">{depre:.1f}%</span> no valor da sua hora.
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    if not st.session_state.get("salvo"):
        try:
            nova_entrada = pd.DataFrame([{
                ...
                "Setor": r["setor"],
                "Mun_Moradia": r["mun_moradia"],
                "Dist_Moradia": r["dist_moradia"],
                "Mun_Trabalho": r["mun_trabalho"],
                "Dist_Trabalho": r["dist_trabalho"],
    ...
    relatorio_txt = (
        "CALCULADORA DO TRECHO - DIAGNOSTICO TECNICO\n"
        ...
        f"Sobra Residual:          R$ {sobra:.2f}\n"
        f"Confisco Total:          R$ {confi:.2f}\n"
        f"Depreciacao Real:        {depre:.1f}%\n"
    )

    st.download_button(
        label="📥 BAIXAR NOTA TÉCNICA (TXT)",
        data=relatorio_txt,
